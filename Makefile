...
.PHONY: help setup setup-gpu setup-colab data features train eval eval-base score serve serve-nvidia weights upload space space-push space-static space-static-push test lint fmt clean all
.DEFAULT_GOAL := help
...
help:
	...
	@echo "make eval      在 test 切分上產生預測並評分（CIDEr + CLIPScore + LLM-as-judge）"
	...
	@echo "make serve-nvidia  啟動 Render/NVIDIA API 輕量互動介面"
	...
	@echo "make space-static  組出免費 Static Space（只展示，不推論）"
	@echo "make space-static-push  直接推送免費 Static Space"
	...
	@echo "               預設 gemma3_4b；上課想更快可用 qwen2_vl_2b"
...
setup-colab:
	...
	pip install -q anthropic pycocoevalcap
	@echo ""
...
# CPU-only：需要 NVIDIA_API_KEY，不下載基礎模型或 LoRA。
serve-nvidia:
	$(PY) -m src.serving.nvidia_app
...
space-static:
	$(PY) -m src.serving.build_space $(ARGS) --static

space-static-push:
	$(PY) -m src.serving.build_space $(ARGS) --static --push
